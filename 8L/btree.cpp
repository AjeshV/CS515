...
BTreeNode * BInternal::insert(int &newKey, string item){
    ...
    int count = 0;
    while( count < currChildren -1 && newKey > keys[count] ){
		count++;
	}
	
	BTreeNode * insert = child[count]->insert(newKey, item);
    
    if(insert){
		
		for(int i = currChildren -1; i>= count+1; i--){
			keys[i+1] = keys[i];
			child[i+1] = child[i];
		}
		
		child[count+1] = insert;
		keys[count+1] = newKey;
		currChildren++;
