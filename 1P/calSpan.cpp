...
for (int j=0; j<20; j++)
{
  string list[20] = {"100.1", "-100.1", "-101", "200", "10", "-1", "139.1"};  
   if (str4 == list[j])
   {
...
for (int i=n2; i<str1.size(); i++)
 {
  string str (str1[i]);
  size_t found = str.find_first_of("abcdfghjklmnopqrstuvwxyz");
   if (found!=string::npos)
   { 
     found=str.find_first_of("abcdfghjklmnopqrstuvwxyz",found+1);   
     cout << num1 << " Data at minute " << n2 << " is corrupted." << endl;
     goto LOOP; 
   }
   else
   { 
